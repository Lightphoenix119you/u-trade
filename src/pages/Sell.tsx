...
export function Sell({ navigate }: SellProps) {
  ...
  // Load hubs for the user's campus
  useState(() => {
    const campusId = profile?.campus_id || selectedCampusId;
    supabase
      .from('campus_hubs')
      .select('*')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error: insertErr } = await supabase
      ...
      .insert({
        seller_id: user.id,
        campus_id: campusId === '00000000-0000-0000-0000-000000000000' ? hubs[0]?.campus_id : campusId,
        hub_id: hubId,

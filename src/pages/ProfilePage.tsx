...
import {
  ...
  ArrowRight, ShieldCheck, PlayCircle, Gift, Copy, Check, MessageCircle,
} from 'lucide-react';
    ...
    <div className="relative z-10 mx-auto max-w-4xl px-4 py-6 sm:px-6">
      ...
      <GlassCard className="mb-6 p-5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">Inviter des amis</h3>
          {profile.referral_benefits_until && new Date(profile.referral_benefits_until) > new Date() && (
        ...
        <p className="mt-1 text-xs text-white/40">
          Partage ton code — chaque ami inscrit avec te fait gagner des avantages vendeur.
        </p>

        <div className="mt-3 flex items-center gap-2">
          <div className="flex-1 truncate rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-center font-mono text-sm font-bold tracking-widest">
            {profile.referral_code || '—'}
          </div>
          <button
            onClick={() => {
              if (!profile.referral_code) return;
              navigator.clipboard.writeText(`https://u-trade-pied.vercel.app/#/signup?ref=${profile.referral_code}`).catch(() => {});
              setReferralCopied(true);
              setTimeout(() => setReferralCopied(false), 1500);
            }}
            className="flex flex-shrink-0 items-center gap-1.5 rounded-lg bg-white/10 px-3 py-2 text-xs font-medium transition hover:bg-white/15"
          >
            {referralCopied ? <><Check className="h-3.5 w-3.5 text-emerald-400" /> Copié</> : <><Copy className="h-3.5 w-3.5" /> Copier</>}
          </button>
          <a
            href={`https://wa.me/?text=${encodeURIComponent(`Rejoins-moi sur U. Trade, le marketplace étudiant ! Utilise mon code ${profile.referral_code} à l'inscription : https://u-trade-pied.vercel.app/#/signup?ref=${profile.referral_code}`)}`}
            target="_blank"
            rel="noreferrer"
            className="flex flex-shrink-0 items-center gap-1.5 rounded-lg bg-[#25D366]/90 px-3 py-2 text-xs font-semibold text-white transition hover:bg-[#25D366]"
          >
        ...
        <div className="mt-3 flex items-center justify-between border-t border-white/10 pt-3 text-xs text-white/40">
          <span>{profile.referral_count} ami{profile.referral_count > 1 ? 's' : ''} parrainé{profile.referral_count > 1 ? 's' : ''}</span>
          {profile.referral_benefits_until && new Date(profile.referral_benefits_until) > new Date() ? (
            <span className="text-emerald-300">Avantages actifs jusqu'au {new Date(profile.referral_benefits_until).toLocaleDateString('fr-FR')}</span>
          ) : (
            <span>Aucun avantage actif</span>
          )}
        </div>
      </GlassCard>

...
import { X, Loader2, AlertCircle, Palette, Tag, Zap, Sparkles, Eye } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/5 p-3">
            ...
            <button
              ...
              className={`relative h-6 w-11 flex-shrink-0 overflow-hidden rounded-full transition ${showViewCount ? 'campus-gradient' : 'bg-white/15'}`}
            >
              <span className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${showViewCount ? 'translate-x-[18px]' : 'translate-x-0'}`} />
            </button>
          </label>
